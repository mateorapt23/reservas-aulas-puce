...
document.addEventListener('DOMContentLoaded', function() {
    ...
    btnEliminarSeleccionados.addEventListener('click', () => {
        ...
        if (!confirm(`¿Eliminar ${ids.length} cátedra(s)?`)) return;
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.message || 'Error al eliminar cátedras');
            }
        })
        .catch(err => {
            console.error(err);
        ...
        document.querySelectorAll(".btn-editar").forEach(btn => {
            btn.onclick = () => {
                ...
                if (seleccionados.size > 1) {
                    alert('No puedes editar mientras tienes varias cátedras seleccionadas.');
                    return;
                }
